...
import { CTKN } from "../contracts/ctkn/CTKN.sol";
...
contract OFTTest is TestHelper {
    ...
    function setUp() public virtual override {
        ...
        aOFT = CTKN(
            _deployOApp(type(CTKN).creationCode, abi.encode("CTKN", "CTKN", address(endpoints[aEid]), address(this)))
        );

        bOFT = CTKN(
            _deployOApp(type(CTKN).creationCode, abi.encode("CTKN", "CTKN", address(endpoints[bEid]), address(this)))
        );
    ...
    function test_send_oft() public {
        ...
        console.logUint(fee.nativeFee);
        console.logUint(fee.lzTokenFee);
    ...
    function test_oft_credit() public {
        ...
        assertEq(aOFT.balanceOf(userA), initialBalance);
        //assertEq(aOFT.balanceOf(address(this)), 0); //we modified the constructor and the delagate has minted tokens
