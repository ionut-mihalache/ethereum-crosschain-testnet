...
leave_docker_swarm_command_remote = (
    "ssh {} 'cd " + abs_path + " ; " + leave_docker_swarm_command + "'"
)
init_docker_swarm_command = leave_docker_swarm_command + " ; docker swarm init"
join_docker_swarm_command = (
    "ssh {} ' cd " + abs_path + " ; " + leave_docker_swarm_command + " ; {}'"
)
start_docker_swarm_command = (
    "DATA_PATH_PREFIX=" + abs_path + " docker stack deploy -c docker-compose.yaml eth"
)
...
def init_node(num_nodes=4, num_chains=0):
    if num_chains == 0:
        return subprocess.Popen(
            init_node_script + " " + str(num_nodes), shell=True
        ).wait()
    else:
        return subprocess.Popen(
            init_node_script + " " + str(num_nodes) + " " + str(num_chains), shell=True
        ).wait()
...
def init_swarm(workers):
    ...
    process_output = (
        subprocess.Popen(
            "docker swarm join-token worker", shell=True, stdout=subprocess.PIPE
        )
        .stdout.read()
        .decode("utf-8")
        .strip()
...
def get_workers(args):
    if args.host_file != "":
        ...
        workers = [
            f"worker-{int(args.start_worker) + i:03}" for i in range(args.num_workers)
        ]
        return workers
