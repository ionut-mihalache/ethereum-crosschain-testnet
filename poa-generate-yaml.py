...
def generate_nodes(chain_number=0, nodes={}):
    for i in range(1,number_node+1):
        ...
        if chain_number==0:
            ...
            volumes.append("${DATA_PATH_PREFIX}/seed_data/genesis" + str(chain_number) + ".json:/root/files/genesis.json:ro")
            volumes.append("${DATA_PATH_PREFIX}/seed_data/node_" + str(i)+'/data' + str(chain_number) + ':/root/data:ro') 
        ...
        command = ("cp -r /root/data /root/data2 ; "
                ...
                "geth --datadir /root/data2 --nodiscover --syncmode full --nodekey /root/files/priv.key --port " + str(port) + " "
                "--http --http.addr \"0.0.0.0\" --http.vhosts=\"*\" --http.corsdomain=\"*\" --http.port " + str(http_port) + " "
